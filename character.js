...
function Character(id, nam, rac, cls, lvl, itm) { 
   ...
   // Currently equipped
   this.Weapon = this.Armor = this.Deck = this.Chip = this.Items.Empty;
   ...
   //------------------------------------ Stats ---------------------------------
   // Vitals   
   this.HP_max = () => roll(10) + roll(10) + roll(10) + this.Level + bonus(this.STM);
   this.SP_max = () => roll(10) + roll(10) + roll(10) + this.Level + bonus(this.INT);
   this.MP_max = () => roll(10) + roll(10) + roll(10) + this.Level + bonus(this.WIL);
   ...
   // Combat bonuses
   ATK_itm = () => {                // Add attribute bonus if weapon / armor equipped
   ...
   DEF_itm = () => this.Armor.Type == "armor" ? this.Athletics + bonus(this.STM) : this.Athletics;
   EVD_itm = () => this.Armor.Type == "armor" ? this.Dodge + bonus(this.STM) : this.Dodge;
   HAX_itm = () => this.Deck.Type == "deck" ? this.Computer + bonus(this.WIT) : this.Computer;
   CPU_itm = () => this.Deck.Type == "deck" ? this.Technology + bonus(this.INT) : this.Technology;
   SEC_itm = () => this.Deck.Type == "deck" ? this.Security + bonus(this.INT) : this.Security;

   this.ATK = ATK_itm();            // Current values
   this.DMG = DMG_itm();
   this.DEF = DEF_itm();
   this.EVD = EVD_itm();
   this.HAX = HAX_itm();
...
// EXPLICIT DECLARATION FOR TESTING ONLY - LIVE: store args in local engine variables for Character creation
var Player = new Character(0, "Player", "European", "Hacker", 1, {});